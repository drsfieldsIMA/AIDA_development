...
const static_paths = [
	{
		...
		content:
			"No longer need these, will be throwing away unless someone wants them.\n" +
			"\n" +
			"3 piece pot set\n" +
			"Wok\n" +
			"Large frying pan\n" +
	...
	},
	{
		...
		content:
			"4 extension leads, various lengths,  three with 4 sockets and one with 2 sockets",
...
export default function RecyclePost({ trash }: { trash: Array<object> | any }) {
	...
	useEffect(() => {
		if (trash) {
			...
			const imgTags = trash?.description?.match(
				/<img [^>]*src="[^"]*"[^>]*>/gm
			);
						...
						<Grid container spacing={2} px={2} marginLeft={0}>
							<Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
								<div
									...
									style={{
										background: `url(${trash.photos[0]?.thumbnail}) 50% 50% no-repeat`,
							...
							<Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
								...
								<Link href={`/browse?${trash.genre}`}>
									<a className='category-link'>{trash.type}</a>
								</Link>
								<Divider
									sx={{
										borderColor: "#00035a",
								...
								<div
									...
									color='#e52d27'
									dangerouslySetInnerHTML={{ __html: `${trash.content}` }}
								/>
...
export async function getStaticPaths() {
	...
	const paths = static_paths.map((post: { post_id: any }) => ({
